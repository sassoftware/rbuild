...
class ShowImagesTest(rbuildhelp.RbuildHelper):
    def testShowImagesCommandParsing(self):
        handle = self.getRbuildHandle(mock.MockObject())
        handle.Show.registerCommands()
        handle.ShowImages.registerCommands()
        handle.ShowImages.initialize()
        cmd = handle.Commands.getCommandClass('show')()
        mock.mockMethod(handle.ShowImages.showImageStatus)
        cmd.runCommand(handle, {}, ['rbuild', 'show', 'images'])
        handle.ShowImages.showImageStatus._mock.assertCalled()
